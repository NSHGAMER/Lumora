      ...
      <div className="glass-panel rounded-2xl px-4 py-3 flex items-center justify-between pointer-events-auto shadow-luxury border border-white/10">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative px-3.5 py-1.5 rounded-lg text-xs font-heading font-medium transition-all duration-200 flex items-center gap-2 cursor-pointer ${
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-900/80 border border-white/10 hover:border-cyan-500/40 text-slate-400 hover:text-slate-200 transition-all text-xs font-mono cursor-pointer"
          >
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-900/90 border border-white/10 hover:border-white/20 text-xs font-mono text-slate-200 transition-all cursor-pointer"
            >
              ...
              <div className="absolute right-0 mt-2 w-36 bg-[#0A1019] border border-white/10 rounded-xl shadow-2xl py-1 z-50">
                {(['student', 'faculty', 'admin'] as SystemRole[]).map((r) => (
                  <button
                    ...
                    className={`w-full px-3 py-1.5 text-left text-xs font-mono capitalize transition-colors flex items-center justify-between cursor-pointer ${
          ...
          <button
            onClick={onOpenJSR}
            className="relative group flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-heading font-medium text-xs shadow-accent-cyan hover:shadow-cyan-500/50 transition-all cursor-pointer overflow-hidden"
          >
